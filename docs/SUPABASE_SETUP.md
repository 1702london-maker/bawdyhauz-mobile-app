...
- `supabase/migrations/0017_advanced_admin_analytics_security.sql`
...
Admin operations that require service-role access should live in Supabase Edge Functions or another server-only backend. Supabase provides reserved runtime variables such as `SUPABASE_SERVICE_ROLE_KEY` automatically to Edge Functions, so do not set `SUPABASE_` secrets manually. Never expose service-role values to Expo.
