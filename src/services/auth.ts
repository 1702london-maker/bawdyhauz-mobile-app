...
export async function requireAdminUser(): Promise<ServiceResult<string | undefined>> {
  if (!supabase) {
    return { data: undefined, mode: supabaseMode };
  }
  ...
  if (!session.data) {
    return { data: undefined, error: session.error, mode: supabaseMode };
  }
  ...
  if (data?.role !== "admin") {
    return { data: undefined, error: "Admin access required.", mode: supabaseMode };
  }

  return { data: session.data, mode: supabaseMode };
}
