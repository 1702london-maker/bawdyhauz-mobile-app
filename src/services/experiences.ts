...
export async function submitExperienceRsvp(
  ...
  if (!error) {
    ...
    await trackAnalyticsEvent("experience.rsvp_requested", "experiences", {
      status: rsvp.status ?? "requested"
    });
  }
...
export async function joinExperienceWaitlist(
  ...
  if (!error) {
    await trackAnalyticsEvent("experience.waitlist_joined", "experiences", {
      preferredCity: waitlist.preferredCity ?? ""
    });
  }
