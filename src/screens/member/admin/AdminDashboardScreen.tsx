...
import { BetaOperationsDashboard } from "./BetaOperationsDashboard";
import { GlobalScalingDashboard } from "./GlobalScalingDashboard";
import { InternalOperationsDashboard } from "./InternalOperationsDashboard";
...
export function AdminDashboardScreen() {
  ...
  const applyApplicationAction = async (
    ...
    setError("");
    setFeedback("");
    const applicant = queues.applications.find((item) => item.id === applicationId);
    const result =
      applicant?.source === "website"
        ? await performWebsiteLeadAction({
            action,
        ...
        : targetUserId
          ...
          : {
              data: false,
              error: "This application is missing a linked user.",
    ...
    setFeedback(`${applicant?.source === "website" ? "Website lead" : "Application"} ${action} recorded.`);
    await refreshQueues();
  };
      ...
      >
        ...
        {queues.applications.map((applicant) => (
          <LuxuryCard key={applicant.id} style={styles.card}>
            ...
            <Body>Source: {applicant.source === "website" ? "Website gateway" : "Mobile app"}</Body>
            {applicant.sourceEmail ? <Body>Email: {applicant.sourceEmail}</Body> : null}
      ...
      <QueueScreen italic="membership status." loading={loading} onBack={() => setView("home")} title="Subscriptions">
        ...
        {subscriptions.map((subscription) => (
          <LuxuryCard key={subscription.id} style={styles.card}>
            ...
            <Body>Billing: {subscription.billingNote ?? "Private billing review"}</Body>
      ...
      <QueueScreen italic="incident queue." loading={loading} onBack={() => setView("home")} title="Safety">
        {queues.safetyReports.map((report) => (
          <LuxuryCard key={report.id} style={styles.card}>
            ...
            <Body>User history: prior notes and reports appear here.</Body>
            <Body>Trust score: {report.trustScore}/100</Body>
            <ActionChips
  ...
  if (view === "moderation") {
    return <ModerationDashboard onBack={() => setView("home")} />;
  }

  if (view === "beta") {
    return <BetaOperationsDashboard onBack={() => setView("home")} />;
  }

  if (view === "operations") {
    return <InternalOperationsDashboard onBack={() => setView("home")} />;
  }

  if (view === "scaling") {
    return <GlobalScalingDashboard onBack={() => setView("home")} />;
  }

  if (view === "analytics") {
    return <AnalyticsDashboard onBack={() => setView("home")} />;
  }

  if (view === "security") {
    return <SecurityDashboard onBack={() => setView("home")} />;
  }
  ...
  return (
    <View>
      ...
      <View style={styles.actions}>
        ...
        <LuxuryButton variant="outline" onPress={() => setView("moderation")}>
          Advanced moderation
        </LuxuryButton>
        <LuxuryButton variant="outline" onPress={() => setView("beta")}>
          Beta launch
        </LuxuryButton>
        <LuxuryButton variant="outline" onPress={() => setView("operations")}>
          Operations tools
        </LuxuryButton>
        <LuxuryButton variant="outline" onPress={() => setView("scaling")}>
          Global scaling
        </LuxuryButton>
        <LuxuryButton variant="outline" onPress={() => setView("analytics")}>
          Analytics
        </LuxuryButton>
        <LuxuryButton variant="outline" onPress={() => setView("security")}>
          Security
        </LuxuryButton>
        <LuxuryButton variant="outline" onPress={() => setView("safety")}>
          Safety reports legacy
        </LuxuryButton>
      </View>
    </View>
  );
