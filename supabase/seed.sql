...
insert into public.user_trust_snapshots (user_id, trust_score, standing, signal_summary, source, created_by_admin_id)
values
  ('33333333-3333-3333-3333-333333333333', 72, 'flagged', 'Seed trust snapshot for moderation dashboard testing.', 'manual_review', '22222222-2222-2222-2222-222222222222')
on conflict do nothing;

insert into public.flagged_behaviour_reviews (target_user_id, signal_type, severity, summary, status)
values
  ('33333333-3333-3333-3333-333333333333', 'profile_mismatch', 'review', 'Seed flagged behaviour review for admin workflow testing.', 'open')
on conflict do nothing;

insert into public.rate_limit_events (user_id, route_key, window_key, event_count, limited)
values
  ('11111111-1111-1111-1111-111111111111', 'auth.sign_in', 'minute', 2, false)
on conflict do nothing;

insert into public.suspicious_activity_events (user_id, signal_type, severity, summary, status)
values
  ('11111111-1111-1111-1111-111111111111', 'rapid_requests', 'review', 'Seed suspicious activity placeholder.', 'open')
on conflict do nothing;

insert into public.abuse_detection_signals (user_id, signal_type, confidence, recommendation, status, ai_review_only)
values
  ('33333333-3333-3333-3333-333333333333', 'safety_anomaly', 0.42, 'Human review only. No automated account action.', 'review_required', true)
on conflict do nothing;

insert into public.device_sessions (user_id, device_label, platform, session_hash, status)
values
  ('11111111-1111-1111-1111-111111111111', 'Seed mobile device', 'ios', 'seed-session-member', 'active')
on conflict (user_id, session_hash) do nothing;
